...
<div class="d-flex flex-wrap gap-2 mb-4 align-items-center">
    <?php foreach (['all', 'approved', 'flagged', 'blocked'] as $tab): ?>
        <?php $q = array_merge($_GET, ['status' => $tab]); unset($q['deleted']); ?>
        <a href="/admin/chat.php?<?php echo http_build_query($q); ?>"
           class="btn btn-sm <?php echo $filterStatus === $tab && !$filterDeleted ? 'btn-ptmd-teal' : 'btn-ptmd-outline'; ?>">
            <?php ee(ucfirst($tab)); ?>
        </a>
    <?php endforeach; ?>
    <?php $dq = array_merge($_GET, ['deleted' => '1', 'status' => 'all']); ?>
...
<?php if ($messages): ?>
    <div class="d-flex flex-column gap-3">
        <?php foreach ($messages as $msg): ?>
            ...
            ?>
            <div class="ptmd-panel p-lg d-flex gap-4 align-items-start <?php echo $isDeleted ? 'opacity-50' : ''; ?>">
